...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isUuid(id) || !['approve','link','reject','request_correction'].includes(String(action))) return NextResponse.json({ error: 'A valid registration action is required.' }, { status: 400 })
